...
description: This guide discusses using Pythons's Dictionary object.
authors: ['Dale Fugier']
...
To construct an instance of a dictionary object with data, that is, key:item pairs filled in,   use one of the the following methods.

The dictionary room_num is created and filled in with each key:value pair, rather than as an empty dictionary. The `key` is a string or number, in the example below it is a person's name,  followed be a colon `:` as a serprator from the associated value which can be any datatype, in this case an integer. Commas `,`  sperate different key:value pairs in the dictionary:
...
To add a value to a Dictionary, specify the new key and set a value. Below, the code creates the dictionary *room_num* with two key:value pairs for Jphn and Liz, then adds a third one for Isaac:
