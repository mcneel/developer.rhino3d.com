...
- [**Eto Custom Dialog Framework**](#eto-custom-dialog-framework).  A cross-platform dialog framework to create more complex modal and non-modal dialogs int a script.
...
1. **Commandline Gets** - GetReal, GetString, GetInteger, GetBoolean, GetColor.
2. **Interactive Gets** - GetPoint(s), GetLine, GetDistance, GetAngle, GetPolyline, GetRectangle, GetBox, GetCursorPos.
3. **Geometry Gets** - GetEdgeCurves, GetMeshFaces, GetMeshVertices, GetPointOnCurve, GetPointOnMesh, GetPointOnSurface.
...
There are 22 different Get methods. For details on all the Get functions in RhinoScriptSyntax for Python go to the [RhinoScriptSyntax User interface methods]({{ site.baseurl }}/api/RhinoScriptSyntax/#userinterface)  
